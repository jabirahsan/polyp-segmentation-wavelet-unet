...
"""Imported From: https://github.com/dmolony3/ResUNet/blob/master/res_unet.py"""
...
def res_unet(input_size=256, num_filters=[64,128,256], kernel_size=(3,3), num_channels=3, num_classes=1):
    """Residual Unet

    Function that generates a residual unet

    Args:
        input_size (int): dimension of the input image. Default:256
        num_filters (list): number of filters for each encoder layer. Defalut: [64,128,256]
        kernel_size (tuple): size of the kernel, applied to all convolutions. Default: (3,3)
        num_channels (int): number of channels for the input image. Default: 3
        num_classes (int): number of output classes for the output. Default: 1 
    Returns:
